  ...
  return (
    <div className="grid">
      <section className="card" style={{ gridColumn: "1 / -1" }}>
        <h2 style={{ marginTop: 0 }}>Team Tasks {admin ? <span className="badge">admin</span> : null}</h2>
        <table>
          <thead>
            <tr>
              <th>Task</th>
            ...
            {tasks?.map(t => {
              ...
              return (
                <tr key={t.id} className={critical ? "critical" : ""}>
                  ...
                  <td>{t.start_date ? format(new Date(t.start_date), "yyyy-MM-dd") : "—"}</td>
                  <td>{t.due_date ? format(new Date(t.due_date), "yyyy-MM-dd") : "—"}</td>
                  <td><span className={`badge status-${t.status.replaceAll(" ", "\\ ")}`}>{t.status}</span></td>
                  <td>
                    <div className="progress"><div style={{ width: `${t.progress ?? 0}%` }} /></div>
